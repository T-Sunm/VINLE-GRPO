...
def evaluate_smile(json_path: str, device: str = "cuda") -> Dict:
    ...
    # Map synthetic answers to their corresponding types
    for item, synth_ans in zip(data, all_synthetic_answers):
        ans_type = item.get("answer_type", "other")
        by_type[ans_type]["synthetic_answers"].append(synth_ans)
    ...
    for ans_type, type_data in by_type.items():
        ...
        if type_data['questions']:
            smile_scores_type = compute_smile_scores(
                ...
                synthetic_answers=type_data['synthetic_answers'],
                model_type='phobert'
            )
